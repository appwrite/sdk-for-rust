//! ColumnVarchar model for Appwrite SDK
...
mod tests {
    ...
    #[test]
    fn test_column_varchar_creation() {
        let _model = <ColumnVarchar as Default>::default();
        let _ = _model.key();
        let _ = _model.r#type();
        let _ = _model.status();
        let _ = _model.error();
    ...
    #[test]
    fn test_column_varchar_serialization() {
        let model = <ColumnVarchar as Default>::default();
        let json = serde_json::to_string(&model);
        assert!(json.is_ok());

        let deserialized: Result<ColumnVarchar, _> = serde_json::from_str(&json.unwrap());
