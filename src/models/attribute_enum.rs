//! AttributeEnum model for Appwrite SDK
...
mod tests {
    ...
    #[test]
    fn test_attribute_enum_creation() {
        let _model = <AttributeEnum as Default>::default();
        let _ = _model.key();
        let _ = _model.r#type();
        let _ = _model.status();
        let _ = _model.error();
    ...
    fn test_attribute_enum_serialization() {
        let model = <AttributeEnum as Default>::default();
        let json = serde_json::to_string(&model);
        assert!(json.is_ok());

        let deserialized: Result<AttributeEnum, _> = serde_json::from_str(&json.unwrap());
