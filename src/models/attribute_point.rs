//! AttributePoint model for Appwrite SDK
...
impl AttributePoint {
    ...
    /// Set default
    pub fn set_default(mut self, default: Vec<f64>) -> Self {
...
mod tests {
    ...
    #[test]
    fn test_attribute_point_creation() {
        let _model = <AttributePoint as Default>::default();
        let _ = _model.key();
        let _ = _model.r#type();
        let _ = _model.status();
        let _ = _model.error();
    ...
    #[test]
    fn test_attribute_point_serialization() {
        let model = <AttributePoint as Default>::default();
        let json = serde_json::to_string(&model);
        assert!(json.is_ok());

        let deserialized: Result<AttributePoint, _> = serde_json::from_str(&json.unwrap());
